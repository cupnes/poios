...
struct EFI_UNICODE_COLLATION_PROTOCOL *UCP;
...
void efi_init(void *ImageHandle, struct EFI_SYSTEM_TABLE *SystemTable)
{
	...
	struct EFI_GUID stiep_guid = {0xdd9e7534, 0x7762, 0x4698, \
				      {0x8c, 0x14, 0xf5, 0x85,	  \
				       0x17, 0xa6, 0x25, 0xaa}};
	struct EFI_GUID ucp_guid = {0xa4c751fc, 0x23ae, 0x4c3e, \
				    {0x92, 0xe9, 0x49, 0x64,	\
				     0xcf, 0x63, 0xf3, 0x49}};
	...
	status = ST->BootServices->OpenProtocol(IH, &lip_guid, (void **)&LIP,
						IH, NULL,
	...
	assert(status, L"LocateProtocol(EFI_SIMPLE_TEXT_INPUT_EX_PROTOCOL)");
	status = ST->BootServices->LocateProtocol(&ucp_guid, NULL, (void **)&UCP);
	assert(status, L"LocateProtocol(EFI_UNICODE_COLLATION_PROTOCOL)");
}
